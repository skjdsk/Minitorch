...
class Network(minitorch.Module):
    ...
    def __init__(self):
        ...
        # Convolution layers
        # Conv1: 1 input channel -> 4 output channels, 3x3 kernel
        self.conv1 = Conv2d(1, 4, 3, 3)
        # Conv2: 4 input channels -> 8 output channels, 3x3 kernel
        self.conv2 = Conv2d(4, 8, 3, 3)

        # After conv layers: BATCH x 8 x 28 x 28
        # After 4x4 pooling: BATCH x 8 x 7 x 7 = BATCH x 392
        self.linear1 = Linear(392, 64)
        self.linear2 = Linear(64, C)

    def forward(self, x):
        # 1. Apply convolution with 4 output channels and 3x3 kernel followed by ReLU
        self.mid = self.conv1.forward(x).relu()

        # 2. Apply convolution with 8 output channels and 3x3 kernel followed by ReLU
        self.out = self.conv2.forward(self.mid).relu()

        # 3. Apply 2D pooling with 4x4 kernel
        pooled = minitorch.avgpool2d(self.out, (4, 4))

        # 4. Flatten channels, height, and width (BATCH x 8 x 7 x 7 -> BATCH x 392)
        batch_size = pooled.shape[0]
        flattened = pooled.contiguous().view(batch_size, 392)

        # 5. Apply Linear to size 64 followed by ReLU and Dropout with rate 25%
        h = self.linear1.forward(flattened).relu()
        h = minitorch.dropout(h, 0.25, not self.training)
...
def default_log_fn(epoch, total_loss, correct, total, losses, model):
    log_msg = f"Epoch {epoch} loss {total_loss} valid acc {correct}/{total}"
    print(log_msg)
...
if __name__ == "__main__":
    ...
    ImageTrain().train(data_train, data_val, learning_rate=0.01, max_epochs=5)