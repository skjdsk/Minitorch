...
def njit(fn: Fn, **kwargs: Any) -> Fn:
    """Compile a function with Numba's `njit` and `inline="always"`.

    Args:
    ----
        fn: Function to compile with Numba.
        **kwargs: Extra options forwarded to `numba.njit`.

    Returns:
    -------
        Compiled function suitable for high-performance tensor ops.
    ...
    for i1 in prange(out_shape[0]):
        for i2 in prange(out_shape[1]):
            for i3 in prange(out_shape[2]):
                # A[i1, i2, :] @ B[i1, :, i3] -> C[i1, i2, i3]
                # Start at A[i1, i2, 0] and B[i1, 0, i3]
                ...
                for _ in range(a_shape[2]):
                    ...
                    a_inner += a_strides[2]  # move along k dimension in A
                    b_inner += b_strides[1]  # move along k dimension in B
